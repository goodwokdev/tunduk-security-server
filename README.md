...
| Флавор | Где | Для чего |
|---|---|---|
| **docker-compose** (корень) | одна виртуалка | опорный флавор - проще всего читать и поднимать |
| **Helm-чарт** ([charts/tunduk-security-server](charts/tunduk-security-server)) | Kubernetes | одиночный production-faithful инстанс в кластере |
...
```
docker-compose.yml   стек из двух сервисов (SS + Postgres 12)
.env.example         шаблон секретов/портов -> скопировать в .env
