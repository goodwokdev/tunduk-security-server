...
Первая коробка - сам сервер безопасности. Контейнер называется security-server, образ - niis/xroad-security-server-sidecar:7.4.2. Образ официальный, его собирает та же организация, что делает весь X-Road (она называется NIIS). Внутри уже крутится около семи мелких служб: одна проксирует запросы, другая подписывает сообщения, третья ведёт мониторинг. Думать про них не надо - всё уже собрано и связано авторами образа, мы просто запускаем готовое.
...
## почему не пришлось собирать свой образ

Когда я впервые увидел "сервер безопасности Тундук", подумал: ну, значит, надо где-то раздобыть исходники Тундука, собрать из них образ, и только потом запускать. Логика очевидная - раз называется по-своему, значит и собирается по-своему.

Оказалось, нет.

Сначала немного контекста. X-Road - это система, через которую государственные информационные системы безопасно обмениваются данными между собой. Тундук - это X-Road, развёрнутый для Кыргызстана. Дальше я буду пользоваться обоими словами как синонимами, потому что внутри это одно и то же.

И это "одно и то же" - не фигура речи. Я залез в репозиторий пакетов Тундука (deb.tunduk.kg) и сравнил его с тем, что выпускает NIIS - организация, которая разрабатывает сам X-Road. Все 28 пакетов совпали байт-в-байт: тот же maintainer, тот же git-хэш сборки. Никакого отдельного "тундуковского" пакета в репозитории нет вообще. Тундук - это не форк X-Road и не переделка. Это зеркало того же репозитория, поднятое внутри страны, чтобы качать пакеты из местной сети, а не из-за рубежа.

Вывод простой: собирать руками нечего. Берём официальный production-образ X-Road как есть и запускаем его.

Тогда возникает резонный вопрос: а что вообще делает наш сервер именно "тундуковским", если программа одинаковая для всех стран?

Делает один файл настройки. Он называется configuration anchor - по-русски "якорь конфигурации". Ты загружаешь его потом, уже через веб-панель сервера (это будет в одной из следующих секций). До загрузки якоря сервер - просто пустой X-Road, который не знает, к какой стране принадлежит.

Проще всего думать о якоре как об адресе главпочтамта. Якорь говорит твоему серверу: вот центральный сервер Тундука, ходи за общими настройками и списками доверенных участников сюда. Загрузил якорь Кыргызстана - сервер стал тундуковским. Загрузил бы якорь другой страны - был бы их.

```
один и тот же образ X-Road
        |
        | + якорь Кыргызстана
        v
   сервер Тундука
...
Что мне в этом нравится с инженерной точки зрения:

- Своего кода у нас почти нет. Нечего поддерживать, нечего чинить, не за чем следить при выходе новых версий X-Road.
- Обновления безопасности приходят простым `docker pull`. Вышла новая версия официального образа - подтянул, перезапустил, и всё. Не надо пересобирать, не надо повторять чужие шаги сборки и надеяться, что ничего не отвалилось.

Скажу честно, где тут подвох: раз образ официальный и общий, то от него и зависит безопасность сервера. За выходом новых версий образа надо следить и накатывать их вовремя - сам себя он не обновит. Но это всё равно несравнимо проще, чем тащить и пересобирать собственную сборку.
