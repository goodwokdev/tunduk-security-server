...
## Порты: наружу или только внутрь

| Service | Порт(ы) | Назначение | Куда | Тип по умолчанию |
|---|---|---|---|---|
| `...-ss-xroad` | 5500, 5577 | обмен между серверами безопасности + OCSP | НАРУЖУ (нужно для работы) | ClusterIP -> переключи на LoadBalancer/NodePort |
| `...-ss-admin` | 4000 | админ-панель | только внутрь (доступ через port-forward) | ClusterIP |
| `...-ss-is` | 8080, 8443 | точки доступа ИС (для своих приложений в кластере) | только внутрь | ClusterIP |

Правило: наружу осознанно выходит только `...-ss-xroad` (5500/5577) - там mTLS, защита
встроена. Панель и точки доступа ИС защиты X-Road не имеют, поэтому остаются ClusterIP.
Если в кластере есть NetworkPolicy или внешний фаервол - пускай снаружи только 5500/5577,
а 4000/8080/8443 закрой и на сетевом уровне (ClusterIP их не публикует, но политику лучше продублировать).
